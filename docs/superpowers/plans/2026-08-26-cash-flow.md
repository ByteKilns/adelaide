...
- [ ] **Step 3b: Fix `SafeToSpendCard`'s `totalActual` prop**

`SafeToSpendCard` independently recomputes its "On track"/"Over budget"
status badge from `totalPlanned - totalActual`, so it must receive the
same netOutflow-adjusted total as `safeToSpend` was computed from —
otherwise the badge and the headline number can disagree. Find:
...
Replace `totalActual={summary.totalExpenses}` with
