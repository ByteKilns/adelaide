...
type ContentProps = { slices: CategorySlice[]; stacked?: boolean; total: number };

// `stacked` forces the donut above the category list instead of side by
// side — used when this renders inside a narrow column (e.g. a tab panel)
// where `sm:flex-row` would still kick in from viewport width alone and
// cramp the two together.
export function ExpenseBreakdownContent({ slices, stacked, total }: ContentProps) {
  return (
    <div className={cn("flex flex-col gap-6", !stacked && "sm:flex-row sm:items-center")}>
...
type Props = { slices: CategorySlice[]; total: number; viewAllHref?: string };

export function ExpenseBreakdownCard({ slices, total, viewAllHref }: Props) {
  return (
    <Card>
      <CardHeader className="flex items-center justify-between pb-2">
        ...
        {viewAllHref && (
          <Link className="flex items-center gap-1 text-sm font-medium text-primary" href={viewAllHref}>
            View full report
      ...
      <CardContent>
        <ExpenseBreakdownContent slices={slices} total={total} />
      </CardContent>
    </Card>
  );
}
